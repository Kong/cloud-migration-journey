...
* First - each microservice pod backed by the mesh is a dataplane. So here we can see there is 1 dp in the cloud zone, i.e 1 pod being supported.

* Second - many of the tags defined on the dp are autogenerated by the the mesh services, but not all. For example, to support canary deployments usually a version tag is added as well to support that logic, that need to be added as a label Kuberentes Deployment Manifest of the Microservice.
...
* **Third** - Integrate a EKS Cluster with our Mesh by leveraging the Kubernetes Deployment Strategy with Multi-zone enabled.

* **Fourth** - Quickly played with the Traffic Management capabilities of Kong Mesh deprecate a monolith service.
